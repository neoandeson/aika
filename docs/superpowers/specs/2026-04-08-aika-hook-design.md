...
│   ├── config.ts                   # Port configuration (AIKA_PORT env, default 4242)
│   ├── commands/
...
│   │   ├── on-stop.mjs             # Stop/SubagentStop — session complete + context generation
...
5. Read journal.jsonl → generate .aika/context.md (session summary + stats)
6. Exit 0
```
...
- **postCapture(payload)** — `fetch("http://localhost:${AIKA_PORT}/api/capture", ...)`. Wrapped in try/catch, silent failure. Fire-and-forget. Port configurable via `AIKA_PORT` env (default 4242).
- **updateContext(cwd)** — Read `journal.jsonl`, split by stop events into sessions, generate `.aika/context.md` with last session summary (commands, files) and all-time stats (sessions, commands, files, kits). No-throw.
- **readStdinJson()** — Read all of stdin, parse as JSON. Returns null on empty/invalid input.
...
### 5.3 context.md — Auto-generated session summary

`.aika/context.md` is regenerated by `on-stop.mjs` on every session end. Git-tracked (rest of `.aika/` is gitignored). Read by `CLAUDE.md` on session resume.

Contains:
- **Last session:** commands run, files touched, transcript path
- **All-time stats:** session count, total commands, total unique files, kits used
